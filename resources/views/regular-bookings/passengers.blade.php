    ...
    <section class="regular-booking-page animate-fade-in">
        <section class="regular-booking-page-header">
            <div class="regular-booking-page-copy">
                ...
                <p>Lengkapi nama dan nomor HP untuk setiap kursi yang sudah dipilih pada tahap sebelumnya.</p>
            </div>
        ...
        @if ($errors->any())
            <section class="regular-booking-feedback-card regular-booking-feedback-card--error">
                <div class="regular-booking-feedback-copy">
                    ...
                    <p>Nama dan nomor HP wajib dilengkapi untuk seluruh kursi yang dipilih.</p>
                </div>
            </section>
        @endif

        <div class="regular-booking-layout">
            <form method="POST" action="{{ route('regular-bookings.passengers.store') }}" class="regular-booking-form-card">
                @csrf

                <section class="regular-booking-section">
                    <div class="regular-booking-section-head">
                        <h2>Form Penumpang</h2>
                        <p>Jumlah form mengikuti kursi terpilih dan urutannya tetap sama dengan urutan kursi pada booking draft.</p>
                    </div>

                    @if ($errors->has('passengers'))
                        <p class="regular-booking-field-error">{{ $errors->first('passengers') }}</p>
                    @endif

                    @if (! $errors->has('passengers') && $errors->has('passengers.*.seat_no'))
                        <p class="regular-booking-field-error">{{ $errors->first('passengers.*.seat_no') }}</p>
                    @endif

                    <div class="regular-booking-passenger-list">
                        @foreach ($passengerFormState['forms'] as $passengerForm)
                            <section class="regular-booking-passenger-card">
                                <div class="regular-booking-passenger-card-head">
                                    <div>
                                        <h3>Penumpang {{ $loop->iteration }}</h3>
                                        <p>Data ini akan dipakai pada tahap review pemesanan.</p>
                                    </div>

                                    <span class="stock-value-badge stock-value-badge-emerald">{{ $passengerForm['seat_no'] }}</span>
                                </div>

                                <input type="hidden" name="passengers[{{ $passengerForm['index'] }}][seat_no]" value="{{ $passengerForm['seat_no'] }}">

                                <div class="regular-booking-grid">
                                    <div class="regular-booking-field">
                                        <label for="regular-booking-passenger-name-{{ $passengerForm['index'] }}">Nama Penumpang</label>
                                        <div class="regular-booking-input-shell">
                                            ...
                                            <input
                                                id="regular-booking-passenger-name-{{ $passengerForm['index'] }}"
                                                type="text"
                                                name="passengers[{{ $passengerForm['index'] }}][name]"
                                        ...
                                        @error('passengers.' . $passengerForm['index'] . '.name')
                                            <p class="regular-booking-field-error">{{ $message }}</p>
                                        @enderror
                                    </div>

                                    <div class="regular-booking-field">
                                        <label for="regular-booking-passenger-phone-{{ $passengerForm['index'] }}">No HP</label>
                                        <div class="regular-booking-input-shell">
                                            <span class="regular-booking-input-icon" aria-hidden="true">
                                                <svg viewBox="0 0 24 24" fill="none">
                                            ...
                                            <input
                                                id="regular-booking-passenger-phone-{{ $passengerForm['index'] }}"
                                                type="tel"
                                                name="passengers[{{ $passengerForm['index'] }}][phone]"
                                        ...
                                        @error('passengers.' . $passengerForm['index'] . '.phone')
                                            <p class="regular-booking-field-error">{{ $message }}</p>
                                        @enderror
                                    </div>
                                </div>
                ...
                <div class="regular-booking-form-actions">
                    <a href="{{ route('regular-bookings.seats') }}" class="dashboard-ghost-button">Kembali</a>
            ...
            <aside class="regular-booking-sidebar">
                <section class="regular-booking-summary-card">
                    <div class="regular-booking-summary-head">
                        ...
                        <p>Ringkasan ini membantu memastikan data penumpang sudah sesuai sebelum lanjut ke tahap review.</p>
                    </div>

                    <div class="regular-booking-summary-grid">
                        <div class="regular-booking-summary-item">
                            <span>Jumlah Kursi Terpilih</span>
                        ...
                        <div class="regular-booking-summary-item">
                            <span>Data Penumpang Terisi</span>
                            <strong>{{ $passengerFormState['filled_count'] }} dari {{ $passengerFormState['required_count'] }}</strong>
                        </div>
                        <div class="regular-booking-summary-item">
                            <span>Rute</span>
                ...
                <section class="regular-booking-note-card">
                    <div class="regular-booking-note-head">
                        <h2>Catatan Data Penumpang</h2>
                        <p>Setelah tahap ini selesai, data penumpang akan langsung dibawa ke halaman review pemesanan.</p>
                    </div>

                    <ul class="regular-booking-note-list">
                        <li>Urutan form selalu mengikuti urutan kursi yang dipilih.</li>
                        <li>Nama dan nomor HP wajib diisi untuk setiap kursi.</li>
                        <li>Nomor HP disimpan dalam format nomor Indonesia agar konsisten pada tahap review berikutnya.</li>
                    </ul>
                </section>
            </aside>
        </div>
