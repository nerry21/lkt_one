...
<section class="ddrop-page animate-fade-in">
    ...
    <div class="ddrop-stats">
        <div class="ddrop-stat">
            <span class="ddrop-stat-label">Total Rental</span>
    ...
    {{-- Search + Filter --}}
    <form method="GET" action="{{ route('rental-data.index') }}" class="ddrop-filter-bar">
        ...
        @if ($search !== '' || $filterStatus !== '')
            <a href="{{ route('rental-data.index') }}" class="ddrop-btn-reset">Reset</a>
        @endif
    </form>
            ...
            <table class="ddrop-table">
                <thead>
                    <tr>
                        ...
                        <th>Nama Pemesan</th>
                        <th>Rute</th>
                        <th>Tgl Pertama Rental</th>
                        <th>Tgl Terakhir Rental</th>
                        <th>Jam Berangkat</th>
                        <th>Jam Berangkat Pulang</th>
                        <th>Tarif Final</th>
                        <th>Kode Mobil</th>
                        <th>Driver</th>
                        <th>Keterangan</th>
                ...
                <tbody>
                    @forelse ($bookings as $booking)
                        @php
                            ...
                            $returnTime     = substr($booking->return_trip_time ?? '', 0, 5);
                            ...
                            $rowData = json_encode([
                                ...
                                'rental_end_date_fmt' => $booking->rental_end_date?->translatedFormat('d F Y') ?? '-',
                                'trip_time'        => $tripTime,
                        ...
                        <tr class="ddrop-tr" data-row='{{ $rowData }}'>
                            ...
                            <td>
                                ...
                                <div class="ddrop-address-hint">{{ Str::limit($booking->pickup_location ?? '', 35) }}</div>
                            </td>
                            <td>
                                <div class="ddrop-date">{{ $booking->trip_date?->translatedFormat('d M Y') ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="ddrop-date">{{ $booking->rental_end_date?->translatedFormat('d M Y') ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="ddrop-time">{{ $tripTime ?: '—' }}{{ $tripTime ? ' WIB' : '' }}</div>
                            </td>
                            <td>
                                <div class="ddrop-time">{{ $returnTime ?: '—' }}{{ $returnTime ? ' WIB' : '' }}</div>
                            </td>
                            ...
                            <td class="ddrop-col-aksi">
                                <div class="ddrop-actions">
                                    ...
                                    <a href="{{ route('rental-data.ticket.download', $booking->id) }}" class="ddrop-action-btn ddrop-action-btn--ticket" title="Download E-Tiket">
                                    ...
                                    <a href="{{ route('rental-data.surat-jalan', $booking->id) }}" class="ddrop-action-btn ddrop-action-btn--sj" title="Download Surat Jalan">
                    ...
                    @empty
                        <tr>
                            <td colspan="12" class="ddrop-empty">
                                ...
                                <p>Belum ada data pemesanan rental mobil.</p>
                                <button type="button" class="ddrop-btn-primary" id="btn-open-create-empty">Tambah Data Rental</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<dialog id="modal-create" class="ddrop-modal">
    <div class="ddrop-modal-box ddrop-modal-box--lg">
        <div class="ddrop-modal-head">
            <h2>Tambah Data Rental Mobil</h2>
        ...
        <form method="POST" action="{{ route('rental-data.store') }}" class="ddrop-modal-form">
            @csrf
            @include('rental-data.partials.form-fields', ['mode' => 'create', 'booking' => null])
...
<dialog id="modal-edit" class="ddrop-modal">
    <div class="ddrop-modal-box ddrop-modal-box--lg">
        ...
        <form method="POST" id="form-edit" action="" class="ddrop-modal-form">
            @csrf
            @method('PUT')
            @include('rental-data.partials.form-fields', ['mode' => 'edit', 'booking' => null])
            <div class="ddrop-modal-actions">
...
<dialog id="modal-show" class="ddrop-modal">
    <div class="ddrop-modal-box ddrop-modal-box--lg">
        <div class="ddrop-modal-head">
            <h2>Detail Pemesanan Rental Mobil</h2>
...
<dialog id="modal-delete" class="ddrop-modal">
    <div class="ddrop-modal-box">
        ...
        <div class="ddrop-delete-body">
            ...
            <p>Anda yakin ingin menghapus data pemesanan rental</p>
...
.ddrop-time  { font-size: 0.85rem; color: #334155; font-weight: 600; }
...
<script>
(function () {
    const BASE_UPDATE_URL = '{{ url('dashboard/rental-data') }}';
    const BASE_DELETE_URL = '{{ url('dashboard/rental-data') }}';
    ...
    document.getElementById('btn-open-create-empty')?.addEventListener('click', () => openModal('modal-create'));
    ...
    function buildDetail(d) {
        ...
        return `
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Kode Booking</span><span class="ddrop-detail-value">${fmt(d.booking_code)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Status Pembayaran</span><span class="ddrop-detail-value">${fmt(d.payment_status)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Nama Pemesan</span><span class="ddrop-detail-value">${fmt(d.passenger_name)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">No HP</span><span class="ddrop-detail-value">${fmt(d.passenger_phone)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Asal</span><span class="ddrop-detail-value">${fmt(d.from_city)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tujuan</span><span class="ddrop-detail-value">${fmt(d.to_city)}</span></div>
            <div class="ddrop-detail-item col-span-2"><span class="ddrop-detail-label">Alamat Penjemputan</span><span class="ddrop-detail-value">${fmt(d.pickup_location)}</span></div>
            <div class="ddrop-detail-item col-span-2"><span class="ddrop-detail-label">Alamat Pengantaran</span><span class="ddrop-detail-value">${fmt(d.dropoff_location)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tgl Pertama Rental</span><span class="ddrop-detail-value">${fmt(d.trip_date_fmt)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tgl Terakhir Rental</span><span class="ddrop-detail-value">${fmt(d.rental_end_date_fmt)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Jam Keberangkatan</span><span class="ddrop-detail-value">${d.trip_time ? d.trip_time + ' WIB' : '—'}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Jam Keberangkatan Pulang</span><span class="ddrop-detail-value">${d.return_trip_time ? d.return_trip_time + ' WIB' : '—'}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tarif Rental</span><span class="ddrop-detail-value">${money(d.price_per_seat)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tambahan Ongkos</span><span class="ddrop-detail-value">${money(d.additional_fare)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Tarif Final</span><span class="ddrop-detail-value">${money(d.total_amount)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Metode Pembayaran</span><span class="ddrop-detail-value">${fmt(d.payment_method)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Driver</span><span class="ddrop-detail-value">${fmt(d.driver_name)}</span></div>
            <div class="ddrop-detail-item"><span class="ddrop-detail-label">Kode Mobil</span><span class="ddrop-detail-value">${fmt(d.kode_mobil)}${d.jenis_mobil ? ' — ' + d.jenis_mobil : ''}</span></div>
            <div class="ddrop-detail-item col-span-2"><span class="ddrop-detail-label">Keterangan</span><span class="ddrop-detail-value">${fmt(d.notes)}</span></div>
        `;
    }
    ...
    function fillEditForm(d) {
        ...
        form.action = BASE_UPDATE_URL + '/' + d.id;
        ...
        set('to_city',          d.to_city);
        set('pickup_location',  d.pickup_location);
        set('dropoff_location', d.dropoff_location);
        set('price_per_seat',   d.price_per_seat);
        set('additional_fare',  d.additional_fare);
        set('trip_date',        d.trip_date);
        set('rental_end_date',  d.rental_end_date);
        set('trip_time',        d.trip_time);
        set('return_trip_time', d.return_trip_time);
        set('payment_method',   d.payment_method);
        set('payment_status',   d.payment_status);
        set('notes',            d.notes);
